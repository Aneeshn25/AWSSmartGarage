...
data "terraform_remote_state" "smart_garage_api_gateway" {
  backend = "s3"
...
resource "aws_cloudfront_distribution" "api_gateway_distribution" {
  origin {
    domain_name = replace(data.terraform_remote_state.smart_garage_api_gateway.outputs.prod_base_url, "/^https?://([^/]*).*/", "$1")
